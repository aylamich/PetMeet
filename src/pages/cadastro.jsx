...
function Cadastro() {
  ...
  const handleSenhaBlur = () => {
    // Quando o usuário sai do campo de senha
    if (!senhaErro) {
      // Se a senha não estiver errada
      setMostrarRequisitosSenha(false); // Os requisitos somem
    }
  };

  const validarEmail = (email) => {
    // Apesar de ter o type EMAIL no input, fiz uma validação extra
    const regexEmail = /^[^\s@]+@[^\s@]+\.[^\s@]+$/; // Regex básico para email
    if (!regexEmail.test(email)) {
      // Se o email não atender ao regex
  ...
  return (
    <div className="h-screen flex flex-col md:flex-row">
      ...
      <div className="w-full md:w-2/5 md:ml-[60%] flex items-start justify-center bg-white overflow-y-auto h-screen">
        {/* Ocupa 2 de 5 blocos na tela em telas médias e grandes e em pequenas toda a largura */}
        <div className="w-full max-w-md p-8">
          ...
          <form id="cadastroForm" onSubmit={handleSubmit}>
            ...
            <div className="mb-4">
              ...
              />
              {dataNascimentoErro && (
                // se der erro renderiza a mensagem
              ...
              {mostrarRequisitosSenha && (
                <div id="senhaRequisitos" className="text-sm mt-2">
                  ...
                  <ul className="list-disc list-inside">
                    <li
                      className={
                        temMinimoCaracteres ? 'text-green-500' : 'text-red-500'
                      }
                    >
                      {/* Se temMinimoCaracteres for verdadeiro, a cor é verde, senão (:), vermelho, mesma lógica pro resto */}
                    ...
                    <li
                      className={temCaractereEspecial ? 'text-green-500' : 'text-red-500'}
                    >
